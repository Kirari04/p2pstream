...
func (a *App) checkPublicCache(r *http.Request, resolution publicRouteResolution) publicCacheDecision {
	...
	if resolution.Backend.BackendType != publicBackendTypeProxyForward {
		return publicCacheDecision{Status: publicCacheStatusBypass, BypassReason: "backend_not_proxy_forward"}
	}
	a.proxyMu.Lock()
...
func publicCacheResponseEligibility(rule publicCacheRuleConfig, resp *http.Response) (time.Duration, []string, bool) {
	...
	for _, token := range strings.Split(cacheControl, ",") {
		...
		if token == "no-store" || token == "private" || token == "no-cache" {
			return 0, nil, false
		}
	}
...
func publicCacheOriginTTL(header http.Header) (time.Duration, bool) {
	...
	for _, part := range strings.Split(cacheControl, ",") {
		...
		for _, prefix := range []string{"s-maxage=", "max-age="} {
			if strings.HasPrefix(part, prefix) {
				seconds, err := strconv.ParseInt(strings.TrimSpace(strings.TrimPrefix(part, prefix)), 10, 64)
	...
	if expires := header.Get("Expires"); expires != "" {
		if t, err := http.ParseTime(expires); err == nil && time.Until(t) > 0 {
...
func publicCacheRuleConfigToProto(rule publicCacheRuleConfig) *p2pstreamv1.PublicCacheRule {
	return &p2pstreamv1.PublicCacheRule{
		...
		BackendIds:           append([]int64(nil), rule.BackendIDs...),
...
func (a *App) validatePublicCacheRuleInput(ctx context.Context, name string, priority int64, enabled bool, routeIDs []int64, backendIDs []int64, scope p2pstreamv1.PublicCacheScope, ttlMode p2pstreamv1.PublicCacheTtlMode, ttlMillis int64, queryMode p2pstreamv1.PublicCacheQueryMode, queryParams []string, varyHeaders []string, statusCodes []int64, maxObjectBytes int64, addCacheStatusHeader bool, allowCookieRequests bool, matchRule *p2pstreamv1.PublicPolicyMatchRule) (publicCacheRuleMutationInput, error) {
	...
	backendIDs, err = a.validatePublicCacheBackendIDs(ctx, backendIDs)
	if err != nil {
		return publicCacheRuleMutationInput{}, err
	}
	...
	backendIDsJSON, _ := json.Marshal(backendIDs)
	...
	return publicCacheRuleMutationInput{
		...
		BackendIDsJSON:       string(backendIDsJSON),
		Scope:                scopeString,
...
func (a *App) validatePublicCacheBackendIDs(ctx context.Context, ids []int64) ([]int64, error) {
	ids = normalizePublicCacheInt64List(ids)
	if len(ids) > maxPublicCacheListItems {
		return nil, connect.NewError(connect.CodeInvalidArgument, errors.New("cache rule has too many backend filters"))
	}
	for _, id := range ids {
		if _, err := a.DB.GetPublicBackend(ctx, id); err != nil {
			return nil, publicDBError(err)
		}
	}
	return ids, nil
...
func (a *App) CreatePublicCacheRule(ctx context.Context, req *connect.Request[p2pstreamv1.CreatePublicCacheRuleRequest]) (*connect.Response[p2pstreamv1.CreatePublicCacheRuleResponse], error) {
	...
	params, err := a.validatePublicCacheRuleInput(ctx, req.Msg.Name, req.Msg.Priority, req.Msg.Enabled, req.Msg.RouteIds, req.Msg.BackendIds, req.Msg.Scope, req.Msg.TtlMode, req.Msg.TtlMillis, req.Msg.QueryMode, req.Msg.QueryParams, req.Msg.VaryHeaders, req.Msg.CacheStatusCodes, req.Msg.MaxObjectBytes, req.Msg.AddCacheStatusHeader, req.Msg.AllowCookieRequests, req.Msg.MatchRule)
	if err != nil {
		return nil, err
	}
