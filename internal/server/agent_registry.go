...
func (a *App) CreateAgent(
	...
	if strings.TrimSpace(req.Msg.PublicId) != "" {
		return nil, connect.NewError(connect.CodeInvalidArgument, errors.New("agent id is generated by the server"))
	}
...
func (a *App) UpdateAgent(
	...
	if publicID := strings.TrimSpace(req.Msg.PublicId); publicID != "" && publicID != existing.PublicID {
		return nil, connect.NewError(connect.CodeInvalidArgument, errors.New("agent id is immutable"))
	}
	if !req.Msg.Enabled {
...
func (a *App) ensureBootstrapAgent(ctx context.Context) {
	...
	if _, err := a.DB.UpsertBootstrapAgent(ctx, db.UpsertBootstrapAgentParams{
		PublicID:  publicID,
		Name:      name,
