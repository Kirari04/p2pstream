...
// Each immutable channel catalog intentionally exposes one target: its latest
// release. Older releases remain usable only for the local
...
// digest exactly matches the current GitHub release manifest.
...
func targetFromVerified(verified *agentupdate.VerifiedCatalog) (*p2pstreamv1.AgentUpdateTarget, error) {
	if verified == nil || verified.Manifest.Sequence > math.MaxInt64 || verified.Manifest.SecurityEpoch > math.MaxInt64 {
