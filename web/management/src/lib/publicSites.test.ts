...
describe("public sites", () => {
  test("requires one exact primary and unique, one-label wildcard aliases", () => {
    const base = { listenerId: "1", name: "App", primaryHostname: "app.example.com", aliases: [] };
    ...
    expect(publicSiteValidationReason({ ...base, primaryHostname: "app\\example.com" })).not.toBe("");
    expect(publicSiteValidationReason({ ...base, primaryHostname: "::1]\\example" })).not.toBe("");
    expect(publicSiteValidationReason({ ...base, primaryHostname: "::1]:443\\" })).not.toBe("");
    ...
    expect(publicSiteValidationReason({
      ...base,
      aliases: [{ id: "1", hostnamePattern: "APP.EXAMPLE.COM.", behavior: PublicSiteHostBehavior.REDIRECT }],
  ...
  test("never turns an unavailable Site into an implicit legacy scope", () => {
    ...
    expect(publicRouteDefaultScopeLabel("unavailable")).toContain("before saving a default route");
  });
});
