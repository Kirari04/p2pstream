...
function editTlsCertificate(certId: bigint) {
  ...
  tlsForm.certPath = cert.certPath;
  tlsForm.keyPath = cert.keyPath;
...
async function submitTlsCertificate() {
  await run(async () => {
    const payload = {
      ...
      certPath: tlsForm.certPath,
      keyPath: tlsForm.keyPath,
...
<template>
  <div v-if="dashboard" class="space-y-8">
    ...
    <section class="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
      <form class="vercel-card p-5" @submit.prevent="submitListener">
        <div class="mb-5 flex items-center justify-between gap-3">
          <h4 class="text-sm font-semibold uppercase tracking-widest text-[#888]">{{ listenerForm.id ? "Edit Listener" : "Add Listener" }}</h4>
          <SecondaryButton v-if="listenerForm.id" size="small" label="Clear" type="button" @click="resetListenerForm" />
        </div>
        ...
        <Button class="mt-5 !bg-white !text-black !border-white" :label="listenerForm.id ? 'Save Listener' : 'Create Listener'" type="submit" :disabled="isBusy || !backends.length" />
      </form>

      <form class="vercel-card p-5" @submit.prevent="submitBackend">
        <div class="mb-5 flex items-center justify-between gap-3">
          <h4 class="text-sm font-semibold uppercase tracking-widest text-[#888]">{{ backendForm.id ? "Edit Backend" : "Add Backend" }}</h4>
          <SecondaryButton v-if="backendForm.id" size="small" label="Clear" type="button" @click="resetBackendForm" />
        </div>
        ...
        <Button class="mt-5 !bg-white !text-black !border-white" :label="backendForm.id ? 'Save Backend' : 'Create Backend'" type="submit" :disabled="isBusy" />

        <div class="mt-6 divide-y divide-[#1f1f1f] border-t border-[#333]">
          <div v-for="backend in backends" :key="backend.id.toString()" class="flex items-center justify-between gap-3 py-3">
            <div class="min-w-0">
              ...
              <p class="truncate text-xs text-[#888]">{{ backendSummary(backend) }}</p>
            </div>
    ...
    <section class="grid gap-6 lg:grid-cols-2">
      <form class="vercel-card p-5" @submit.prevent="submitRoute">
        <div class="mb-5 flex items-center justify-between gap-3">
          <h4 class="text-sm font-semibold uppercase tracking-widest text-[#888]">{{ routeForm.id ? "Edit Route" : "Add Route" }}</h4>
          <SecondaryButton v-if="routeForm.id" size="small" label="Clear" type="button" @click="resetRouteForm" />
        </div>
        <div class="grid gap-4 sm:grid-cols-2">
          <label class="grid gap-1.5 text-xs font-medium uppercase tracking-wider text-[#888]">
            Listener
        ...
        <Button class="mt-5 !bg-white !text-black !border-white" :label="routeForm.id ? 'Save Route' : 'Create Route'" type="submit" :disabled="isBusy || !listeners.length || !backends.length" />
      </form>
    ...
    <section class="grid gap-6 lg:grid-cols-2">
      <form class="vercel-card p-5" @submit.prevent="submitTlsCertificate">
        <div class="mb-5 flex items-center justify-between gap-3">
          <h4 class="text-sm font-semibold uppercase tracking-widest text-[#888]">{{ tlsForm.id ? "Edit TLS Mapping" : "Add TLS Mapping" }}</h4>
          <SecondaryButton v-if="tlsForm.id" size="small" label="Clear" type="button" @click="resetTlsForm" />
        </div>
        <div class="grid gap-4">
          <label class="grid gap-1.5 text-xs font-medium uppercase tracking-wider text-[#888]">
            HTTPS listener
          ...
          <label class="grid gap-1.5 text-xs font-medium uppercase tracking-wider text-[#888]">
            Certificate path
            <input v-model="tlsForm.certPath" class="vercel-input text-sm normal-case tracking-normal" required />
          </label>
          <label class="grid gap-1.5 text-xs font-medium uppercase tracking-wider text-[#888]">
            Key path
            <input v-model="tlsForm.keyPath" class="vercel-input text-sm normal-case tracking-normal" required />
          </label>
          <label class="flex items-center gap-2 text-sm text-[#d4d4d8]">
        ...
        <Button class="mt-5 !bg-white !text-black !border-white" :label="tlsForm.id ? 'Save TLS Mapping' : 'Create TLS Mapping'" type="submit" :disabled="isBusy || !httpsListeners.length" />
      </form>
        ...
        <div class="divide-y divide-[#1f1f1f]">
          <div v-for="cert in tlsCertificates" :key="cert.id.toString()" class="grid gap-3 px-5 py-4 sm:grid-cols-[1fr_auto]">
            <div class="min-w-0">
              ...
              <p class="truncate font-mono text-xs text-[#888]">{{ cert.certPath }}</p>
            </div>
            <div class="flex gap-2">
