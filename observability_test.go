...
func TestE2E_GetDashboardSummaries(t *testing.T) {
	...
	insertProxyEventAt(t, database, now.Add(-2*time.Minute), http.StatusOK, 100, "")
	insertProxyEventAt(t, database, now.Add(-30*time.Minute), http.StatusNotFound, 200, "")
	insertProxyEventAt(t, database, now.Add(-2*time.Hour), http.StatusBadGateway, 300, "")
	insertProxyEventAt(t, database, now.Add(-2*time.Minute), http.StatusGatewayTimeout, 400, "agent_timeout")
	...
	oneHour := windows["1h"]
	if oneHour.ProxyRequests != 3 || oneHour.ProxyClientError != 1 || oneHour.ProxyAvgDurationMs != 233 {
...
func TestProxyRequestEventRecordedCountsOnly(t *testing.T) {
	...
	expected := []string{"duration_ms", "error_kind", "id", "occurred_at", "status_code"}
