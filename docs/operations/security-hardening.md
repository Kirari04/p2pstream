...
2. Protect `/data`:
   ...
   - Treat Vault Transit as a startup dependency. Steady-state proxy traffic uses the cached in-memory snapshot, and repeated reloads can reuse the bounded in-process DEK cache while entries remain valid. Cold cache starts, cache expiry, changed wrapped DEKs, and restarts still need Vault to unwrap stored DEKs.
