...
For an already enrolled host, repair refreshes the rescue updater and agent configuration while preserving the current managed agent slot and rollback state. It does not upgrade the running agent binary. Use **Agents → Updates → Plan rollout** to upgrade that agent. On staging, the target matches the selected management server's release; update that server first when moving to a newer staging release.
