...
Compatible peers establish four parallel TCP tunnels by default. They share one agent identity and resource budget; dropping one lane preserves traffic on the others. Reusable HTTP transports distribute requests across these lanes, including HTTP/2 requests to the same origin. Receive windows grow on demand with memory reservations. See [Agent proxy performance](../operations/agent-proxy-performance) for tuning and measured results.
...
The **Agents** page puts enabled offline agents and version differences first, then supports search by name, ID, version, or label and filtering by state or version difference. The Fleet tab shows each agent's reported version beside the selected management server version. Stable releases and immutable SemVer prereleases are ordered using SemVer and marked **Update available** or **Ahead of server**. Development, nightly, legacy mutable staging, and custom builds are only marked as different when their version or reported commit differs because their upgrade direction cannot be inferred safely. A mutable-tag agent that predates commit reporting is marked **Build unverified** instead of being assumed current. The same table shows current connected or offline duration, uptime percentage over the observability retention window, connection and disconnect counts, load, and visible **Investigate** and **Edit** actions. Exact identity and selector details are collapsed until needed; lifecycle actions such as enable/disable, rotate, uninstall, and delete live in each row's **More** menu. The Activity tab shows the newest connection sessions and is filtered automatically when **Investigate** is selected. Uptime is calculated from retained `connections` rows, clipped to the retention window and the agent creation time. If the management server restarts after a crash or forced stop, stale open connection rows are closed at startup so old sessions do not appear active forever.
