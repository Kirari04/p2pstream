...
Choose **Templates** under **Configure** in the management UI. Summary cards show total, generic, captcha, and waiting-room counts. The compact template table shows name and description, kind and content type, usage count, required placeholders, updated time, and actions.
...
5. Save the template, then select it from the relevant rate-limit or WAF rule drawer. The API supports selecting a generic template for a static route target, but the current route drawer exposes inline static bodies only; configure that association through the management API.

The **New Generic Body**, **New Captcha Page**, and **New Waiting Room** shortcuts below the table open the same drawer with the corresponding kind preselected.
