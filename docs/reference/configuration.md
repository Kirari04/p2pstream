...
| `TUNNEL_MAX_CONCURRENT_REQUESTS` | `64`                         | Sets independent server ceilings for logical public agent-route requests and physical live/closing Yamux streams. The stream ceiling is partitioned into pooled, burst, and protected health capacity. |
...
- `TUNNEL_MAX_CONCURRENT_REQUESTS` must be between `1` and `2048`. Its product with `TUNNEL_MAX_STREAM_WINDOW_BYTES` cannot exceed `536870912` bytes. The server holds a physical slot from Yamux open through peer FIN (including pooled idle and closing streams), and separately applies the same configured number as the backward-compatible logical public agent-request ceiling. The agent applies its own independent request/stream ceiling.
- At the default server total of `64`, four streams are reserved for trusted health checks, public traffic can use `60`, and at most `45` public streams may remain pool-eligible. The remaining `15` public slots are structural cold-target/burst headroom. When multiple agent sessions are connected, one session may use at most `45` public physical streams, preserving an aggregate `15` for demand from other sessions; this is not a separate 15-stream entitlement for every agent, and already-active streams are never preempted. Queued work is round-robin across routes, skips a head waiter blocked only by its session, and can reclaim same-session or global idle capacity as appropriate. A single connected session may borrow the full public budget. If the configured total exceeds Yamux's per-session open backlog, the server also reserves the health share inside that backlog so a public opening burst cannot consume every pending-open position.
