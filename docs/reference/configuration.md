...
| `TUNNEL_MAX_STREAM_WINDOW_BYTES` | `67108864` | Maximum Yamux receive credit per stream. Starts at up to `512 KiB` and grows under sustained transfer when memory can be reserved. |
...
| `TUNNEL_MAX_STREAM_WINDOW_BYTES`  | Maximum Yamux receive window per tunnel stream. Defaults to `67108864`. |
