...
| `AGENT_UPDATES_ENABLED`          | `false`                      | Enables the trusted signed-release catalog and managed update campaign API. Campaign creation remains fail-closed unless the configured root authenticates the selected channel. |
...
| `AGENT_UPDATE_CATALOG_REFRESH_MILLIS` | `300000`              | Signed catalog refresh interval; range `10000`–`3600000`. A last-known-good target is retained through transient network errors only until its signed expiry. |
...
- Managed updates require a pinned `owner/repo`, channel, unexpired threshold root, and management HTTPS. Each catalog accepts only canonical signed manifests for its own stable or staging channel and its own monotonic security/version floors; mutable tags, cross-channel releases, arbitrary URLs, and unknown artifacts are never eligible campaign targets.
