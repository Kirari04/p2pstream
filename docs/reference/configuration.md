...
| `TUNNEL_MAX_STREAM_WINDOW_BYTES` | `1073741824` | Maximum Yamux receive credit per stream. Starts at up to `512 KiB`; measured consumption and session RTT guide growth, with additional storage reserved first. |
...
| `TUNNEL_MAX_STREAM_WINDOW_BYTES`  | Maximum Yamux receive window per tunnel stream. Defaults to `1073741824`. |
...
| `TUNNEL_UPSTREAM_SOCKET_BUFFER_BYTES` | Origin TCP buffering: default `0` enables Linux kernel autotuning, with its possible queues reserved separately for the socket lifetime. A positive `16384`–`16777216` requests a fixed per-direction buffer, charged at four times the value for both directions and Linux doubling. CLI: `--tunnel-upstream-socket-buffer-bytes`. |
...
- `TUNNEL_MAX_CONCURRENT_REQUESTS` must be between `1` and `2048` when explicitly set on an agent. Upgrade server and agent together to use the current adaptive capacity headers. When the value is absent, matching current peers use adaptive admission with `65536` only as an implementation guard. An explicit fixed value remains supported; an older agent that omits capacity remains fixed at `64`.
...
- Every stream initially reserves its receive credit (up to `512 KiB`) plus socket/relay overhead. Two sustained consumption samples, a smoothed consumption rate, and the recent minimum session RTT guide credit toward eight bandwidth-delay products, using at least 2 ms for relay scheduling, bounded by `TUNNEL_MAX_STREAM_WINDOW_BYTES` and at most one doubling per decision. Each growth reserves the additional bounded receive chunks and metadata first. Denied growth leaves the stream serving at its current window. Credit remains reserved through local close until peer FIN or forced Yamux cleanup. Idle connections keep their already-advertised credit, but do not grow just because they have transferred many bytes. Yamux receive storage uses lazy 64 KiB chunks and retains at most 64 drained chunks (4 MiB) per stream for reuse; this is not preallocated and does not change the resource reservation bound. The `1 GiB` ceiling is a protocol limit; the shared memory budget can deny growth earlier. After a pressured agent generation fully drains, the agent asynchronously returns unused Go heap pages to the OS.
...
`CONFIG_DIR` is created or tightened to `0700`. The managed certificate directory is `${CONFIG_DIR}/certs`; an enabled managed GeoLite2 Country database is stored under `${CONFIG_DIR}/geoip`, whose directory and database file are tightened to `0700` and `0600`. SQLite directories created by p2pstream use `0700`, and database/WAL/SHM files are set to `0600`. When `DATABASE_URL` points into an existing directory, p2pstream preserves that directory's mode; secure that directory and its backups separately. If the default `${CONFIG_DIR}/p2pstream.db` is absent while a legacy working-directory `p2pstream.db` exists, startup fails with an unsupported-location error. Move the database into `${CONFIG_DIR}` or set `DATABASE_URL` explicitly; no automatic database copying is performed.
...
Declared or streamed bodies above the configured body ceiling receive `413`, and stalled uploads receive `408`. Long-lived responses and continuously progressing uploads have no total-duration timeout. Accepted public sockets are resource-accounted before TLS and header parsing. Each logical request additionally reserves twice the header limit plus `64 KiB` for bounded request state; HTTP/2 uploads also reserve up to their `1 MiB` receive credit (or their smaller declared body length); direct origin connections reserve socket/relay memory and descriptors. Byte-only reservations for HTTP/2 requests, retries and growing receive windows do not consume imaginary file descriptors. Explicit per-peer guards also apply their resource fair-share policy; the default zero disables that optional policy.
